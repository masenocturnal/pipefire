...
func TestRecordEncryptionCreate(t *testing.T) {
	...
	// db, err := db.ConnectToDb(pipelineConfig.Database)
	// if err != nil {
	// 	t.Error(err.Error())
	// }

	// recorder := encryption_recorder.NewEncryptionRecorder(db, log.WithField("test", "true"))
	...
	// txn := db.Begin()
	// // create a new record
	// rec := &encryption_recorder.EncryptionRecord{
...
func TestRecordEncryptionUpdate(t *testing.T) {
	...
	// recorder := encryption_recorder.NewEncryptionRecorder(db, logger)
